...
mod tests {
    ...
    fn init_test_roots() -> &'static TestRoots {
        TEST_ROOTS.get_or_init(|| {
            ...
            let base = std::env::temp_dir().join(format!("oxsync-tests-{}-{}", std::process::id(), unique));
    ...
    async fn create_event_copies_ten_thousand_small_files_across_nested_directories() {
        ...
        for dir_index in 0..100 {
            for file_index in 0..100 {
                let path = source_case.join(format!(
                    "tree/dir-{dir_index:03}/file-{file_index:03}.txt"
                ));
        ...
        for source_path in &file_paths {
            ...
            assert!(target_path.is_file(), "missing target file {:?}", target_path);
    ...
    async fn modify_event_updates_nested_file_contents_without_touching_siblings() {
        ...
        FileOperationsManager::copy(
            &mut store,
            Instant::now(),
            modify_event([original.clone()]),
        )
        .await;
